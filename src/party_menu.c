...
void CursorCb_MoveItemCallback(u8 taskId)
{
    ...
    case 1:     // User hit A on a Pokemon
        ...
        if(GetMonData(&gPlayerParty[gPartyMenu.slotId2], MON_DATA_HELD_ITEM) >= ITEM_RUBY_MAIL && GetMonData(&gPlayerParty[gPartyMenu.slotId2], MON_DATA_HELD_ITEM) <= ITEM_RETRO_MAIL)
        {
            PlaySE(SE_FAILURE);
            return;
        }
