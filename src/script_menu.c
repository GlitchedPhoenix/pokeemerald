...
static void CreateFossilMultichoice(void)
{
	...
	for (j = 0; j < FOSSIL_SELECTION_COUNT; j++)
	{
		...
		if (selection != 0xFF)
		{
			pixelWidth = DisplayTextAndGetWidth(sFossilOptions[selection], pixelWidth);
		}
	}
	...
	for (selectionCount = 0, i = 0; i < FOSSIL_SELECTION_COUNT; i++)
	{
		...
		{
			AddTextPrinterParameterized(windowId, FONT_NORMAL, sFossilOptions[sFossilSelections[i]], 8, selectionCount * 16 + 1, TEXT_SKIP_DRAW, NULL);
			selectionCount++;
		}
	}
